function count = cprintf(style,format,returnString,varargin)
...
% $Revision: 1.10 $  $Date: 2015/06/24 01:29:18 $
  ...
  if ~exist('returnString','var') || ~islogical(returnString)
    returnString = false;
  end
  ...
  if isdeployed | ~useDesktop %#ok<OR2> - for Matlab 6 compatibility
      ...
      if returnString
        count1 = sprintf(format,varargin{:});
      else
...
function [underlineFlag,boldFlag,style,debugFlag] = processStyleInfo(style)
  ...
  % Style name
      ...
      matlabStyles = {'Text','Keywords','Comments','Strings','UnterminatedStrings','SystemCommands','Errors', 'Warning'};
      validStyles  = [matlabStyles, ...
                      'Black','Cyan','Magenta','Blue','Green','Red','Yellow','White', 'Red', ...
                      'Hyperlinks'];
      matches = find(strncmpi(style,validStyles,length(style)));

      % No match - error
      if isempty(matches)
          fprintf('\ntYMA:cprintf:InvalidStyle: Invalid style - see help section for a list of valid style values, defaulting to warning style\n')
          style = 'Warning';
          matches = find(strncmpi(style,validStyles,length(style)));

      % Too many matches (ambiguous) - error
      elseif length(matches) > 1
          fprintf('\n\tYMA:cprintf:AmbigStyle: Ambiguous style name - supply extra characters for uniqueness, defaulting to warning style\n');
          style = 'Warning';
          matches = find(strncmpi(style,validStyles,length(style)));
